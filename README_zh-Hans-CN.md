...
- **Unicode 支持（预计在 0.4.1 版本发布）**：Frosty 和 FsLocalization 格式不支持大于 `0xFFFF` 的字符（也就是 **表情符号**、部分 **生僻汉字**、**历史与古文字** 和 **特殊符号与字母**）。保存项目/模组时，Frosty 会将过大的字符写成乱码。Flammenwerfer 会将这些字符保存为 UTF-8 编码，并在之后覆盖，从而支持这些字符。这样您的字符串/注释就不会出现乱码。但由于 FsLocalization 格式的限制，过长的字符无法应用到游戏中。
- **ID 数据库（预计在 0.4.1 版本发布）：** 一个带有可视化界面的字符串 ID 数据库，提升您的模组开发体验。
   - **项目 ID 数据库：** 位于您项目中的数据库。它会将所有您添加的 ID 以单独的 已修改 EBX 直接保存到您的项目文件中。无论是独自使用还是与他人协作都非常好用。您还可以手动为每个 ID 添加注释和引用。
   - **缓存 ID 数据库：** 位于 Frosty 的 `Caches` 文件夹中的数据库。它会缓存游戏中所有字符串的 ID。还有一个引用部分，用于查看引用字符串 ID 的文件。
      - **搜索游戏文件：** 一个加强版搜索功能，源自 Localized String Editor 的 Export String Usage List 功能。通过尝试所有属性的可能性、在 Swf 文件中搜索、以及猜测 EBX 文件名是否是 ID，可以比旧版多解析出约 20% 的字符串 ID。（在 PvZGW2 中，原版：23426 中解析出 12585 个。火焰喷射器：23426 中解析出约 16000 个）
      - **手动观察：** 移除所有字符串后，游戏将直接显示字符串 ID，您可以手动将 ID 添加到 ID 数据库中。
         > [!TIP]
         > 您可以使用“批量修改字符串”窗口移除所有字符串，勾选`使用正则表达式`并输入`.*`
      - **加速计算：** 使用 GPU 猜测字符串 ID。此功能在未来计划中。
      - **共享：** 缓存的 ID 数据库是一个 txt 文件。您可以与他人共享您精心制作的 ID 数据库，而他人可以直接使用或合并两个数据库。
