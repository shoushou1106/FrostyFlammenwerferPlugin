...
我需要模组 (Mod) 创作者、普通用户以及资深火焰法师来协助测试此插件。仅靠几个人无法保证稳定性。

### 如何贡献测试
1. 请访问我们的 [Project](https://github.com/users/shoushou1106/projects/3) 查看开发路线图和当前进度。
2. 寻找处于 **"In review"** 状态的项目。
3. 对这些功能进行测试，并在对应的 Issue 中报告您的结果，告知我是否正常工作，或者有任何问题。非常感谢您的贡献！

如果您想参与测试，但遇到困难或不清楚测试流程，可以联系我：
- **Discord:** `shoushou1106`
- **QQ:** `2420308592`
...
- **自动清理**：当没有任何修改内容时，喷火器插件会自动清理已修改的资源，而不是继续将一个语言标记为已修改。

### 扩展功能

- **喷火器接口 (API)**：一个允许其他插件访问喷火器扩展功能的侧通道。
- **字符串移除**：有总比没有好。移除一个字符串将使游戏显示原始 `ID_`。
- **Unicode 支持**：Frosty 和 FsLocalization 格式不支持大于 `0xFFFF` 的字符（也就是 **表情符号**、部分 **生僻汉字**、**历史与古文字** 和 **特殊符号与字母**）。保存项目/模组时，Frosty 会将过大的字符写成乱码。Flammenwerfer 会将这些字符保存为 UTF-8 编码，并在之后覆盖，从而支持这些字符。这样您的字符串/注释就不会出现乱码。但由于 FsLocalization 格式的限制，这些字符无法应用到游戏中。
- **ID 数据库**：一个带有编辑器标签页的字符串 ID 数据库，提升您的模组开发体验。
   - **缓存 ID 数据库：** 位于 Frosty 的 `Caches` 文件夹中的数据库。它会缓存游戏中所有字符串的 ID，并提供用于查看哪些文件引用了字符串 ID 的引用部分。
      - **搜索游戏文件：** 一个加强版搜索功能，源自 Localized String Editor 的“Export String Usage List”。通过尝试更多属性的可能性、在 Swf 文件中解析、猜测 EBX 文件名是否为 ID、还有扩展已知排列，可以比原版多解析出大约 15% 更多的字符串 ID。（例如，在 PvZGW2 中，原版在 23,426 中解析出 12,585 个，火焰喷射器揭示了 15,615 个。）
      - **手动观察：** 移除所有字符串后，游戏将回退显示字符串的 ID。然后您就可以手动将 ID 添加到 ID 数据库中。

      - **加速计算：** 使用 GPU 猜测字符串 ID。*（此功能在未来计划中）*
   - **项目 ID 数据库：** 另一个位于您项目中的数据库。它将您添加的所有 ID 以单独的 EBX 直接保存到项目文件中（位于 `Flammenwerfer/ProjectIdDatabase`）。永远不会丢失，即使项目在之后被 `FsLocalizationPlugin` 触碰。无论是独自使用还是与他人协作都非常好用。您还可以为每个 ID 手动记录引用，或者在 `Tools > Options > Flammenwerfer Options` 中禁用。*（未来计划添加注释功能）*
   - **共享：** 两个数据库使用相同且易于阅读的 JSON 格式，并使用统一的导入按钮。缓存数据库本身就是一个 JSON 文件，而项目数据库可以导出为 JSON。您可以轻松地共享、合并，并使用 Git（或其他版本控制）来跟踪更改。其他创作者可以导入您精心制作的缓存数据库，您也可以与协作者无缝同步项目数据库。

> [!TIP]
> 您可以使用“批量修改字符串”窗口移除所有字符串。勾选`使用正则表达式`，输入`.*`，并点击 移除。
...
- 由于 FsLocalization 格式的限制，**无法支持**大于 `0xFFFF` 的字符。详见上方 **Unicode 支持**。
